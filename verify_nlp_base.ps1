...
Get-ChildItem -Path $vault -Recurse -Filter "*.md" | ForEach-Object {
    ...
    $contentMoc  = $text -match 'MOC - NLP & Psychology'

    if ($tagNLP -or $tagNLPPsy -or $tagPsych -or $nameNLP -or $contentMoc) {
        # Skip the MOC itself
        if ($f.Name -notlike 'MOC - NLP*') {
            ...
            $results += [PSCustomObject]@{
                ...
                Folder    = if ($inFolder) { 'NLP_Psy' } else { ($f.Directory.Name) }
                Reason    = $reason -join ', '
            }
        }
    }
...
Write-Host "=== INSIDE NLP_Psy folder ==="
$results | Where-Object { $_.Folder -eq 'NLP_Psy' } | Sort-Object File | Format-Table File,Reason -AutoSize
