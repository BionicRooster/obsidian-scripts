...
Write-Output "Cleaning up duplicate tags..."
Write-Output ""
...
foreach ($fileName in $uniqueFiles) {
    ...
    # Check if has multiple frontmatter or duplicate tags
    $originalContent = $content
    $changesMade = $false

    # Remove any duplicate "tags: [NLP_Psy]" lines in the body
    if ($content -match '---[\s\S]*?---') {
        $match = [regex]::Match($content, '(---[\s\S]*?---)')
        if ($match.Success) {
            $frontmatter = $match.Groups[1].Value
            $body = $content.Substring($match.Groups[1].Length)

            # Remove duplicate "tags: [NLP_Psy]" from body
            $newBody = $body -replace '^\s*tags:\s*\[NLP_Psy\]\s*', ''
    ...
    # Also remove any duplicate frontmatter blocks
    $frontmatterCount = ([regex]::Matches($content, '^---')).Count
    if ($frontmatterCount -gt 2) {
        # Has multiple frontmatter blocks, rebuild
        $match = [regex]::Match($content, '(^---[\s\S]*?^---)', [System.Text.RegularExpressions.RegexOptions]::Multiline)
        if ($match.Success) {
            $firstFrontmatter = $match.Value
            $restOfContent = $content.Substring($match.Length)

            # Remove all "---" and "tags:" lines from the rest
            $restOfContent = $restOfContent -replace '^---\s*$', ''
            $restOfContent = $restOfContent -replace '^\s*tags:\s*\[NLP_Psy\]\s*$', ''
            $restOfContent = $restOfContent -replace '^\s*tags:\s*$', ''
            $restOfContent = $restOfContent -replace '^\s*- NLP\s*$', ''

            $content = $firstFrontmatter + "`n" + $restOfContent
            $changesMade = $true
        }
    }

    # Write back if changes were made
    if ($changesMade -and $content -ne $originalContent) {
...
Write-Output ""
Write-Output "Cleaned: $cleaned files"
